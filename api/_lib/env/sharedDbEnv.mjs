...
export function resolveSharedDbTier(env = getDefaultEnv()) {
...
export function validateOptionalSharedDbTierLabel(env = getDefaultEnv()) {
...
export function resolveLandscapeSlot(env = getDefaultEnv()) {
...
export function logLandscapeContext(appId, env = getDefaultEnv()) {
...
export function inferSharedDbTierFromDatabaseUrl(dbUrl, env = getDefaultEnv()) {
...
export function validateSharedDbUrlMatchesDeploymentTier(env = getDefaultEnv(), dbUrl) {
...
export function runKinetixSharedDbBootstrapOnce(env = getDefaultEnv()) {
  if (bootstrapDone) {
    return
  }
  bootstrapDone = true
  ...
  const tier = resolveSharedDbTier(env)
  const resolved = resolveKinetixRuntimeEnvFromObject(env)
  logLandscapeContext('kinetix', env)
  validateSharedDbUrlMatchesDeploymentTier(env, resolved.supabaseUrl)
  const strictTier = env.SHARED_DB_ENV_STRICT === '1' || env.SHARED_DB_ENV_STRICT === 'true'
  if ((strictTier || tier === 'prod') && !resolved.supabaseUrl) {
    throw new Error(
      'Missing Kinetix Supabase URL. Set SUPABASE_URL (or VITE_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_URL).'
    )
  }
  if ((strictTier || tier === 'prod') && !resolved.supabaseServiceRoleKey) {
    throw new Error(
      'Missing Kinetix Supabase server key. Set SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_SERVICE_KEY).'
    )
  }
  const supabaseHost = safeUrlHost(resolved.supabaseUrl)
  const anonFp = fingerprintSecretMaterial(resolved.supabaseAnonKey)
  const serviceFp = fingerprintSecretMaterial(resolved.supabaseServiceRoleKey)
  console.warn(
    `[shared-db] kinetix tier=${tier} vercelEnv=${env.VERCEL_ENV ?? 'unset'} ` +
      `supabaseHost=${supabaseHost ?? 'none'} publishableFp=${anonFp} serviceKeyFp=${serviceFp}`
  )
}
