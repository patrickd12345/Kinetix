...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!code || !codeVerifier) {
    return sendApiError(res, 400, 'code and code_verifier required', { source: req.headers })
  }
  ...
  if (!clientId || !clientSecret) {
    logApiEvent('error', 'kinetix_garmin_oauth_missing_config', {
      message: 'Garmin Connect OAuth not configured',
    })
  ...
  try {
    const token = await exchangeGarminAuthorizationCode({
      ...
      clientId,
      clientSecret,
      redirectUri: typeof redirectUri === 'string' && redirectUri.startsWith('http') ? redirectUri : undefined,
    })
    ...
    if (error instanceof GarminAuthError) {
      logApiEvent('error', 'kinetix_garmin_oauth_exchange_failed', {
    ...
    return sendApiError(res, 500, 'Failed to exchange Garmin authorization code', {
      source: req.headers,
