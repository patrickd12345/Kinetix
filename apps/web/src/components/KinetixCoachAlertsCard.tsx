...
export function KinetixCoachAlertsCard({ loading, error, alerts, insufficientData }: KinetixCoachAlertsCardProps) {
  if (loading) return <section className="glass rounded-2xl p-5 border border-white/10"><p className="text-sm text-gray-400">Checking coaching alerts…</p></section>
  ...
  if (insufficientData || alerts.alerts.length === 0) return <section className="glass rounded-2xl p-5 border border-white/10"><p className="text-sm text-gray-400">No coaching alerts right now.</p></section>

  return (
    <section className="glass rounded-2xl p-5 border border-amber-500/20 space-y-3" aria-label="Coaching alerts">
      <header>
        <h3 className="text-lg font-black text-white">Coaching Alerts</h3>
      </header>
