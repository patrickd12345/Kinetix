...
export function KinetixGoalProbabilityCard({
  ...
  if (loading) {
    return (
      <section className="glass rounded-2xl p-5 border border-white/10">
        <p className="text-sm text-slate-600 dark:text-gray-400">Computing goal probability…</p>
      </section>
    )
  }
  if (error) {
  ...
  if (insufficientData || !goalProbability) {
    return (
      <section className="glass rounded-2xl p-5 border border-white/10">
        <p className="text-sm text-slate-600 dark:text-gray-400">Set a race goal with progress to see goal probability.</p>
      </section>
    )
  }
  ...
  return (
    <section className="glass rounded-2xl p-5 border border-emerald-500/25 space-y-3" aria-label="Goal probability">
      <header>
        <h3 className="text-lg font-black text-slate-900 dark:text-white">Goal probability</h3>
        <p className="text-xs text-slate-600 dark:text-gray-400">Deterministic blend of prediction, readiness, simulation, timeline, goal progress, and coach memory (no AI).</p>
      </header>
      <div className="flex items-baseline justify-between gap-3">
        <span className="text-4xl font-black text-slate-900 dark:text-white tabular-nums">{gp.probability}</span>
        <span className="text-xs text-slate-600 dark:text-gray-400 shrink-0">/ 100</span>
      </div>
      ...
      <p className="text-sm text-slate-800 dark:text-gray-200 leading-relaxed">{gp.summary}</p>
    </section>
  )
}
