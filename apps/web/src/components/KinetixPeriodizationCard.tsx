...
export function KinetixPeriodizationCard({
  ...
  if (loading) {
    return (
      <section className="glass rounded-2xl p-5 border border-white/10" aria-live="polite">
        <p className="text-sm text-slate-600 dark:text-gray-400">Computing adaptive periodization…</p>
      </section>
    )
  }
  ...
  return (
    <section className="glass rounded-2xl p-5 border border-indigo-500/20 space-y-4" aria-label="Kinetix periodization">
      <header>
        <h3 className="text-lg font-black text-slate-900 dark:text-white">Adaptive Periodization</h3>
      ...
      <dl className="space-y-2 text-sm">
        <div className="flex items-center justify-between gap-4">
          <dt className="text-slate-600 dark:text-gray-400">Phase</dt>
          <dd className="font-semibold text-slate-900 dark:text-white">{titleCase(periodization.phase)}</dd>
        </div>
        <div className="flex items-center justify-between gap-4">
          <dt className="text-slate-600 dark:text-gray-400">Weeks remaining</dt>
          <dd className="font-semibold text-slate-900 dark:text-white">{periodization.weeksRemaining}</dd>
        </div>
        <div className="flex items-center justify-between gap-4">
          <dt className="text-slate-600 dark:text-gray-400">Next phase</dt>
      ...
      <p className="text-xs text-indigo-900 dark:text-indigo-200">{periodization.focus}</p>
    </section>
  )
}
