...
export function KinetixRaceSimulationCard({ loading, error, simulation }: KinetixRaceSimulationCardProps) {
  if (loading) {
    return (
      <section className="glass rounded-2xl p-5 border border-white/10" aria-live="polite">
        <p className="text-sm text-slate-600 dark:text-gray-400">Running race simulation…</p>
      </section>
    )
  }
  ...
  return (
    <section className="glass rounded-2xl p-5 border border-amber-500/20 space-y-4" aria-label="Kinetix race simulation">
      ...
      {simulation.caution && (
        <div className="rounded-lg border border-yellow-600/40 bg-yellow-500/10 px-3 py-2 text-xs text-yellow-900 dark:text-yellow-200">
      ...
      <dl className="space-y-2 text-sm">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <dd className="font-semibold text-slate-900 dark:text-white">{simulation.confidenceLabel}</dd>
        </div>
      </dl>

      <p className="text-xs text-amber-900 dark:text-amber-200">{simulation.pacingRecommendation}</p>

      <div>
        <p className="mb-2 text-xs uppercase tracking-wide text-slate-600 dark:text-gray-400">Splits</p>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="text-slate-600 dark:text-gray-400">
              <tr>
            ...
            <tbody>
              {simulation.splits.map((split) => (
                <tr key={split.label} className="border-t border-white/10">
                  <td className="py-1.5 pr-2 text-slate-700 dark:text-gray-300">{split.label}</td>
                  <td className="py-1.5 pr-2 text-slate-900 dark:text-white">{split.paceFormatted}</td>
                  <td className="py-1.5 text-slate-900 dark:text-white">{split.cumulativeFormatted}</td>
                </tr>
              ))}
            </tbody>
          </table>
