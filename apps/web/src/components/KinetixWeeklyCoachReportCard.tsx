...
export function KinetixWeeklyCoachReportCard({ loading, error, report, insufficientData }: KinetixWeeklyCoachReportCardProps) {
  if (loading) return <section className="glass rounded-2xl p-5 border border-white/10"><p className="text-sm text-slate-600 dark:text-gray-400">Building weekly coach report…</p></section>
  ...
  if (insufficientData || !report) return <section className="glass rounded-2xl p-5 border border-white/10"><p className="text-sm text-slate-600 dark:text-gray-400">Not enough data for weekly coach report yet.</p></section>
  ...
  return (
    <section className="glass rounded-2xl p-5 border border-sky-500/20 space-y-3" aria-label="Weekly coach report">
      <header>
        ...
        <p className="text-xs text-sky-900 dark:text-sky-100">{report.summary}</p>
      </header>
      <dl className="space-y-2 text-sm">
        {report.sections.map((section) => (
          <div key={section.label} className="flex items-start justify-between gap-4 border-b border-white/5 pb-1">
            <dt className="text-slate-600 dark:text-gray-400">{section.label}</dt>
            <dd className="text-slate-900 dark:text-white text-right">{section.value}</dd>
          </div>
        ))}
      </dl>
    </section>
