...
Integration Wave 1 Status: Blocked (irreducible external npm registry prerequisite)
...
Wave 1 blocker classification:
- Product verification blockers: none confirmed (assertions not yet executed).
- Workspace/tooling blockers: workspace graph is repaired with local `@bookiji-inc/*` stubs, but npm registry access still fails (`ERR_PNPM_FETCH_403`). Current irreducible Wave 1 runtime prerequisite is access to `jsdom@24.1.3` and testing-library artifacts from `registry.npmjs.org` (or equivalent internal mirror).
- Operational runbook: see `docs/KINETIX_LOCAL_VERIFICATION_BASELINE.md`.
