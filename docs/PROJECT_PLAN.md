...
**Last reviewed:** 2026-04-29 — Branch `kx-mvp-beta-fasttrack`: MVP beta web (compliance pages, footer, cookie banner, env readiness, beta full-access flags), AdSense `ads.txt` + Infisical/Vercel `VITE_ADSENSE_CLIENT`, [`ADSENSE_GO_LIVE.md`](deployment/ADSENSE_GO_LIVE.md), Vercel rewrite excludes `ads.txt` / CSP AdSense hosts. Previous: 2026-04-27 PM — Lazy-loaded heavy web routes (**688989f**) to keep main JS chunk under bundle budget; production probes green. **`GET /api/entitlements`** + **`POST /api/platform-profile/sync`** implemented for iOS Lane B (`api/entitlements/index.ts`, `api/platform-profile/sync/index.ts`). Vitest **480** tests / **107** files. Phase 4 day-1 closure prep run by agent earlier same day. Lane A automated gates re-confirmed PASS (lint, type-check, Vitest **476** tests / 106 files at that checkpoint). Lane A6-7 Playwright suite **43 passed / 3 skipped / 0 failed** (4-worker cap); CI workflow added at [`.github/workflows/web-e2e.yml`](../.github/workflows/web-e2e.yml). Day-1 artifacts shipped: SSO closure script, Stripe live readiness script, entitlement toggle migration, interactive runbook, Stripe cutover doc patch. Lane B (native store-ready) **complete** on `feat/native-store-ready` (worktree `Kinetix-native`): all seven steps shipped (B1 secrets/PrivacyInfo, B2 Supabase auth + entitlements, B3 reader-app posture, B4 simulated-Garmin removed, B5 push dropped + Sentry, B6 native CI, B7 ASC submission package). Lane C (Garmin Connect) parked on partner approval — [`GARMIN_CONNECT_APPLICATION_BRIEF.md`](GARMIN_CONNECT_APPLICATION_BRIEF.md) + [`GARMIN_POST_APPROVAL_RUNBOOK.md`](GARMIN_POST_APPROVAL_RUNBOOK.md). Lane D (ops + comms) docs published: [`INCIDENT_RUNBOOK.md`](INCIDENT_RUNBOOK.md), [`STATUS_PAGE_SETUP.md`](STATUS_PAGE_SETUP.md), [`PRIVACY_TOS_LAUNCH_REVIEW.md`](PRIVACY_TOS_LAUNCH_REVIEW.md), [`PHASE4_LAUNCH_COMMS.md`](PHASE4_LAUNCH_COMMS.md). Operator action queue (eight rows) lives in [`PHASE4_RELEASE_EVIDENCE.md`](PHASE4_RELEASE_EVIDENCE.md) and [`PHASE4_INTERACTIVE_RUNBOOK.md`](PHASE4_INTERACTIVE_RUNBOOK.md). Previous: 2026-04-11 — optional `VITE_AUTH_REDIRECT_URL` and [`authRedirect.ts`](../apps/web/src/lib/authRedirect.ts) ([`ENV_PARITY.md`](deployment/ENV_PARITY.md)). 2026-04-10 — Wave 2 web **CLOSED**; Phase 4 scripted gates green; production anonymous curl + Infisical prod **PASS**; `/api/support-queue/*` list `GET` JSON **403**; Kinetix prod magic link sign-in verified OK for pilot.

**Current phase:** Phase 4 (Release Readiness) — day-1 artifacts shipped, operator interactive sign-off remaining
**Wave 2 (web):** Closed
...
- **Serverless API (`api/`):** Vercel functions — billing checkout, Strava/Withings OAuth helpers, **`GET /api/entitlements`**, **`POST /api/platform-profile/sync`** (native handoff), `ai-chat` / `ai-coach`, admlog (non-prod), support-queue (merged entry), shared `_lib` (Stripe, Supabase JWT user, AI/memory boundary). As of 2026-04-27 confirm **Hobby function count** in Vercel dashboard after adding native endpoints; see `vercel.json` rewrites.
...
- Vercel **Hobby** serverless footprint was reduced (historical target **~12** handlers excluding `_lib`; additional routes such as Garmin OAuth/refresh **added later** — verify total deployed functions against the current `api/` tree and Vercel plan limits): smoke script moved out of `api/`, Withings oauth+refresh merged to `api/withings` with rewrites preserving client URLs, support-queue consolidated per [`PHASE4_RELEASE_EVIDENCE.md`](PHASE4_RELEASE_EVIDENCE.md).
- **Wave 2 web closure (CLOSED):** [`docs/KINETIX_SCOPE_CLOSURE.md`](KINETIX_SCOPE_CLOSURE.md) records per-surface closure criteria, CLOSED status, feature freeze, and Phase 3-5 follow-through; [`docs/KINETIX_LOCAL_VERIFICATION_BASELINE.md`](KINETIX_LOCAL_VERIFICATION_BASELINE.md) records the closure-day `@kinetix/web` Vitest run (**346** tests, 2026-04-10) plus lint/type-check and a **current** snapshot section; coaching context hooks consolidated to satisfy `react-hooks/rules-of-hooks` (merged `useKinetixCoachingContext` + `useKinetixCoachingContextState`).
...
- **KX-FEAT-006 (Garmin Connect cloud OAuth scaffolding):** Server (`api/garmin-oauth`, `api/garmin-refresh`, `api/_lib/garminOAuth.ts` + tests), client (`apps/web/src/lib/garminConnectPkce.ts`, `garminConnect.ts`, `useGarminConnectAuth.ts`), Settings UI, Vite dev OAuth proxy. Live activation parked on Garmin Developer Program approval per [`GARMIN_CONNECT_DEVELOPER_PROGRAM.md`](GARMIN_CONNECT_DEVELOPER_PROGRAM.md). Post-approval rollout: [`GARMIN_POST_APPROVAL_RUNBOOK.md`](GARMIN_POST_APPROVAL_RUNBOOK.md).
- **Phase 4 Lane A6-7 (Playwright + CI):** Local Playwright suite green on 4 workers (43 passed / 3 skipped); GitHub Actions workflow [`.github/workflows/web-e2e.yml`](../.github/workflows/web-e2e.yml) runs Playwright on push + PR for `apps/web`/`api`/`packages/core`/`monorepo-packages` paths.
- **Phase 4 Lane B (native store-ready):** Branch `feat/native-store-ready` ships secret strip + iPhone PrivacyInfo, Supabase auth + entitlement gating on iOS, reader-app subscription link, simulated-Garmin removal, Sentry Cocoa instead of push, native CI workflow, and ASC submission package. Operator follow-ups: device audit + TestFlight per [`IOS_LAUNCH_CHECKLIST.md`](IOS_LAUNCH_CHECKLIST.md).
- **Phase 4 Lane D (operations + comms):** Incident runbook, status page setup, privacy/ToS launch review, and launch comms templates published.
