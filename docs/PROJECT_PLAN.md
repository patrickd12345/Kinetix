...
- **Serverless API (`api/`):** Vercel functions — billing checkout, Strava/Withings OAuth helpers, `ai-chat` / `ai-coach`, admlog (non-prod), support-queue (merged entry), shared `_lib` (Stripe, Supabase JWT user, AI/memory boundary). As of 2026-04-10 the **route** `.ts` count under `api/` (excluding `_lib`) is **10**, within the Vercel Hobby cap. See `vercel.json` rewrites.
