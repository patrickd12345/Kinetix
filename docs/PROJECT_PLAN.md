...
**Last reviewed:** 2026-05-01 — BKI-104 added Kinetix web user-evidence crawl (`pnpm test:crawl`, `.github/workflows/web-crawl.yml`, [`docs/testing/WEB_PLAYWRIGHT_CRAWL.md`](testing/WEB_PLAYWRIGHT_CRAWL.md)) alongside iOS Maestro crawl. Previous: 2026-04-30 — Planning docs reconciled with [`KINETIX_SCOPE_CLOSURE.md`](KINETIX_SCOPE_CLOSURE.md): **Current phase = Phase 4 (Release Readiness)**, while **Phase 3 (Platform Hardening)** remains active as a **parallel non-release-blocking** track. Verified 2026-04-30 automation state: [`pnpm verify:kinetix-parity`](../package.json) **PASS**; [`bash watchos/scripts/verify-native-cli.sh`](../watchos/scripts/verify-native-cli.sh) **PASS**; `KinetixPhoneTests` **15 tests, 0 failures**; Watch compile gate (`KinetixWatch` target, `watchsimulator`) **PASS**. Evidence is captured in [`PHASE4_RELEASE_EVIDENCE.md`](PHASE4_RELEASE_EVIDENCE.md) under **Automation preflight (2026-04-30, go-live plan)** and mirrored by the note in [`PHASE4_INTERACTIVE_RUNBOOK.md`](PHASE4_INTERACTIVE_RUNBOOK.md). Withings key-name hygiene was rechecked via `pnpm infisical:list-keys`: `/kinetix` includes `VITE_WITHINGS_CLIENT_ID` and `WITHINGS_*` names (no `WITINGS_*` typo), with live key validation still operator-owned after `infisical login`. Physical Watch smoke remains GUI-only per [`docs/kinetix/KX-SMOKE-013-real-device-smoke.md`](kinetix/KX-SMOKE-013-real-device-smoke.md).

**Current phase:** Phase 4 (Release Readiness) — automated preflight PASS; operator interactive sign-off remaining
**Wave 2 (web):** Closed
...
- Lane B release actions: physical iPhone + paired Watch smoke in Xcode GUI and TestFlight / App Store Connect submission path.
...
- **Phase 4 Lane A6-7 (Playwright + CI):** Local Playwright suite green on 4 workers (43 passed / 3 skipped); GitHub Actions workflow [`.github/workflows/web-e2e.yml`](../.github/workflows/web-e2e.yml) runs Playwright on push + PR for `apps/web`/`api`/`packages/core`/`monorepo-packages` paths. User-evidence crawl now has a named path: `pnpm test:crawl`, [`.github/workflows/web-crawl.yml`](../.github/workflows/web-crawl.yml), and [`docs/testing/WEB_PLAYWRIGHT_CRAWL.md`](testing/WEB_PLAYWRIGHT_CRAWL.md).
