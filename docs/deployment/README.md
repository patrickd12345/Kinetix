...
| Infisical paths, merge order, `pnpm dev`, `pnpm dev:infisical`, `pnpm verify:infisical` | [INFISICAL_LOCAL_DEV.md](./INFISICAL_LOCAL_DEV.md) |
...
3. **Secrets:** Infisical **`/platform`** + **`/kinetix`**, merged with Kinetix overrides; `pnpm dev` uses this flow by default, and `pnpm verify:infisical` validates it from the Kinetix repo root.
