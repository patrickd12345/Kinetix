...
- **History empty state:** Flow `40-history-empty` must show a readable empty state (title + description), not low-contrast text on the dark background. The **large navigation title** must also stay readable on the dark chrome (`preferredColorScheme(.dark)`, opaque `toolbarBackground`, `.toolbarColorScheme(.dark, for: .navigationBar)`). After changing `HistoryView` or global list styling, re-run the iOS Crawl workflow and treat **completed** CI as the proof; manually canceled runs are not sufficient verification. **Evidence (2026-05-01):** run `25225263675` on `main` — 5/5 flows SUCCESS; artifact skim showed empty-state body OK but large “History” title still murky on bar + Settings Diagnostics caption tight to tab bar — follow-up native commits raise tab-bar clearance to **72pt** and tighten History chrome. **Re-verify:** run `25226015173` on `main` at `97c229d` — 5/5 SUCCESS; screenshots `40-history-empty-state.png` + `20-settings-scroll-4.png` show readable empty-state copy and improved spacing above the floating tab bar on Settings tail.
