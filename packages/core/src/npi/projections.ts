import { calculateNPI, UserProfile, RunData } from './calculator';
...
/**
 * Calculate time-to-beat projection based on current run data and target NPI
 */
export function calculateTimeToBeat(
  ...
  // Calculate current NPI
  const currentRunData: RunData = {
    distanceKm: currentDistanceKm,
    timeSeconds: currentTimeSeconds,
    unit: 'metric'
  };
  const currentNPI = calculateNPI(currentRunData, userProfile);
  ...
  // Projection logic: calculate distance needed to reach target NPI
