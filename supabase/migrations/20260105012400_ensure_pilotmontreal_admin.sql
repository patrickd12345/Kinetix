...
-- Create trigger function that will grant admin role when this user registers
...
-- Now handle existing user if they exist
DO $$
DECLARE
  v_user_id UUID;
  v_app_user_id UUID;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'User not found for email: % - trigger created to auto-grant admin role when they register', v_email;
  END IF;
END $$;
